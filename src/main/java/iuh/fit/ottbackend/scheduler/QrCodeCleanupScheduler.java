...
public class QrCodeCleanupScheduler {
    ...
    /**
     * Chạy mỗi 5 phút để cleanup QR codes đã hết hạn
     */
    @Scheduled(cron = "0 */5 * * * *")
    @Transactional
    public void cleanupExpiredQrCodes() {
        ...
        try {
            // Tìm và update status của QR codes đã hết hạn
    ...
    /**
     * Chạy mỗi giờ để cleanup sessions đã hết hạn
     */
    @Scheduled(cron = "0 0 * * * *")
    @Transactional
    ...
    /**
     * Chạy mỗi ngày lúc 3 giờ sáng để cleanup login history cũ
     */
    ...
    public void cleanupOldLoginHistory() {
        ...
        try {
            // Xóa login history cũ hơn 90 ngày
